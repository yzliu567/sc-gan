...
class Coach:
    def __init__(self, opts, prev_train_checkpoint=None):
        ...
        if prev_train_checkpoint is not None:
            ...
            self.ffhqminglasses, self.ffhqmaxglasses, self.ffhqminage, self.ffhqmaxage, self.ffhqminpose, self.ffhqmaxpose = \
                np.loadtxt('../distances/glassesagepose_min_max.txt')
        elif self.opts.mmd:
            if 'afhq' in self.opts.dataset_type:
                print('loading afhq e4e')
                self.minage, self.maxage = np.loadtxt('../distances/catage_minmax.txt')
                self.minyaw, self.maxyaw = np.loadtxt('../distances/catyaw_minmax.txt')
            else:
                self.ffhqminglasses, self.ffhqmaxglasses, self.ffhqminage, self.ffhqmaxage, self.ffhqminpose, self.ffhqmaxpose = \
                    np.loadtxt('../distances/glassesagepose_min_max.txt')
            self.load_to_finetune()
            #del ckpt

    def load_to_finetune(self, ckpt=None):
        print('Loading to finetune...')
        self.global_step = 200001
        if self.opts.progressive_steps:
            self.check_for_progressive_training_update(is_resume_from_ckpt=True)
        print(f'Finetune training from step {self.global_step}')
    ...
    def deg2label(self, deg, modes, min_scale, max_scale, device="cuda"):
        label = torch.zeros([modes], dtype=torch.float32).to(device)
        bounds = torch.linspace(min_scale, max_scale, modes).numpy()

        if deg < bounds[0]:
            label[0] = 1
        elif deg >= bounds[modes - 1]:
            label[modes - 1] = 1
        else:
            for j in range(modes - 1):
                if deg < bounds[j + 1]:
                    label[j + 1] = (deg - bounds[j]) / (bounds[j + 1] - bounds[j])
                    label[j] = 1 - label[j + 1]
                    break
        return label

    def getsoftlabel(self, pose, modes, min_scale, max_scale, device="cuda"):
        batch = pose.shape[0]
        label = torch.empty([batch, modes], dtype=torch.float32).to(device)
        for j in range(batch):
            label[j] = self.deg2label(pose[j], modes, min_scale, max_scale, device)
        return label

    def get_modes(self, real_glasses, real_hats, real_pose):
        device="cuda"
        batch = real_glasses.shape[0]
        real_glasses = real_glasses.to(device)
        real_hats = real_hats.to(device)
        real_pose = real_pose.to(device)
        '''
        for j in range(batch):
            if real_glasses[j] > -37.6:
                real_glasses[j] = -35.0
            else:
                real_glasses[j] = -40.0
        '''
        real_label = torch.zeros([batch, self.opts.modes]).to("cuda").to(torch.float32)
        real_label[:, 0] = 1.0
        if 'afhq' in self.opts.dataset_type:
            #real_label[:, 1:4] = self.getsoftlabel(real_hats, 3, self.minage, self.maxage)
            real_label[:, 4:7] = self.getsoftlabel(real_pose, 3, self.minyaw, self.maxyaw)
        else:
            real_label[:, 1:4] = self.getsoftlabel(real_hats, 3, self.ffhqminage, self.ffhqmaxage)
            real_label[:, 4:7] = self.getsoftlabel(real_pose, 3, self.ffhqminpose, self.ffhqmaxpose)
        real_label[:, 2] = 0
        real_label[:, 5] = 0
        if self.opts.modes > 7:
            real_label[:, 7:10] = self.getsoftlabel(real_glasses, 3, self.ffhqminglasses, self.ffhqmaxglasses)
            real_label[:, 8] = 0
        return real_label
    ...
    def forward(self, batch):
        if not self.opts.mmd:
            ...
            x, y, is_glasses, is_hats, pose_val = batch
            x, y = x.to(self.device).float(), y.to(self.device).float()
            df_modes = self.get_modes(is_glasses, is_hats, pose_val)
            '''
            with torch.no_grad():
                #_, df_modes = self.net.gandisc(x, need_class=True)
                df_modes = df_modes.detach()
                soft_max = torch.nn.Softmax()
                df_modes[:,1:4] = soft_max(df_modes[:,1:4])
                df_modes[:, 4:7] = soft_max(df_modes[:, 4:7])
                df_modes[:, 7:10] = soft_max(df_modes[:, 7:10])
                df_modes[:,0] = 1
                df_modes[:,2] = 0
                df_modes[:,5] = 0
                df_modes[:,8] = 0
            ...
            y_hat, latent = self.net.forward(x, return_latents=True, default_modes=df_modes)
            if self.opts.dataset_type == "cars_encode":
                y_hat = y_hat[:, :, 32:224, :]
            return x, y, y_hat, latent
