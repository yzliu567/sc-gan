...
def luma_flip_mat(axis, i):
    ...
    flip = 2 * torch.ger(axis, axis) * i.view(-1, 1, 1).to(torch.float32)
...
def sample_affine(p, size, height, width, device="cpu"):
    ...
    Gc = rotate_mat(-math.pi / 2 * param.to(torch.float32), device=device)
    G = random_mat_apply(p, Gc, G, eye, device=device)
...
def get_padding(G, height, width, kernel_size):
    ...
    pad = pad.min(torch.tensor([float(width - 1), float(height - 1)] * 2, device=device))
...
def random_apply_affine(img, p, G=None, antialiasing_kernel=SYM6):
    ...
    grid = F.affine_grid(G_inv[:, :2, :].to(img_2x), shape)
